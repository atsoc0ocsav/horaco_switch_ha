...
The switch exposes cumulative frame counters and nothing else — no byte or
octet counters exist anywhere in this firmware, so a bit rate cannot be
derived, only a frame rate. See ``docs`` in the README for why no bits/sec
sensor is synthesised.
